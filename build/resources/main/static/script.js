...
async function updateOrdersBelowDivForVehicle(vehicleId){
  ...
  inputOrderID.placeholder = "Enter Order ID";
  inputOrderID.style.marginLeft = "20px";
  orderDiv.appendChild(inputOrderID);
  ...
  submitButton.onclick = async function() {
    const orderId = inputOrderID.value;
    try {
      const response = await fetch("http://localhost:9000/vehicle/order/" + vehicleId + "/" + orderId ,{
          method: 'POST'
      });
      console.log(response);
      if (response.ok) {
          showCorrectMessage("Vehicle/Order Relation Added");
  ...
  orderDiv.appendChild(submitButton);
  ...
  discardButton.onclick = async function() {
    const orderId = inputOrderID.value;
    try {
      const response = await fetch("http://localhost:9000/vehicle/order/" + orderId ,{
          method: 'DELETE'
      });
      console.log(response);
      if (response.ok) {
        showCorrectMessage("Vehicle/Order Relation Removed");
        updateOrdersBelowDivForVehicle(vehicleId);
      } else {
          console.error('Error loading vehicle:', response.status);
          showWarningMessage("There is no order with this ID");
  ...
  orderDiv.appendChild(discardButton);*/
}
